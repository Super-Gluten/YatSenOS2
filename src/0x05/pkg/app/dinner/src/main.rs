...
fn main() -> isize {
    ...
    let help = r#"
        ...
        函数3：要求哲学家必须按照筷子从小到大拿去，会出现不公平的现象。\n
        请输入对应调用函数序号：
    "#;
    println!("{}", help);
    ...
    match stdin().read_line().trim() {
        ...
        "3" => {
            println!("函数3：要求哲学家必须按照筷子从小到大拿去，会出现不公平的现象。");
            for i in 0..PHI_SIZE {
                let pid = sys_fork();
                if pid == 0 {
                    philosopher3(i);
...
// 函数3：要求哲学家必须按照筷子从小到大拿取，会出现不公平的现象。
fn philosopher3(i: usize) {
    ...
    for _a in 0..10 {
        ...
        if left > right {
            ...
            right = left ^ right;
            left = left ^ right;
        } // 如果左手的筷子比右手的筷子大，可以认为两者需要交换
        // 即对于4号哲学家而言，要先拿右手的筷子，再拿取左手的筷子
        CHOPSTICK[left].wait();
